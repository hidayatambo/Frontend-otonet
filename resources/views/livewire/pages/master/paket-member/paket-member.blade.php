        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header pb-0 card-no-border">
                    <a href="{{ url('master/paket_member/add_paket_member') }}" class="btn btn-primary btn-sm mb-2" id="show_create">Create Paket Member</a>
                </div>
                <div class="card-body">
    ...
    $(document).ready(function () {
        thisTable = $('#paketDatatable').DataTable({
            ...
            ajax: function (data, callback, settings) {
                $.ajax({
                    url: apiUrl + "paketmember/datatable",
                    type: 'GET',
                    data: {
                        ...data,
                    },
            ...
            columns: [{
                    data: "kode_paketmember",
                },
                {
                    data: "nama_paketmember",
                },
                {
                    data: "tipe_paket",
                },
                {
                    ...
                    render: function (data, type, row) {
                        return `<a wire:navigate href="{{ url('master/paket_member/detail_paket_member/${row.id}') }}" class="cursor-pointer view-karyawan" id="view_karyawan" data-id="${row.id}" title="Buat Akun"><i class="fa fa-file-o text-success fa-lg mx-2"></i></a>
                        <a class="cursor-pointer hapus-paket-member" data-id="${row.id}" title="Delete"><i class="fa fa-trash-o text-danger fa-lg mx-2"></i></a>`;
                    }
                },
            ]
        });
        // hapus data
        $("#paketDatatable").on("click", ".hapus-paket-member", function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // console.log();
                    $.ajax({
                        url: apiUrl + "paketmember/" + currentPaketId,
                        type: "DELETE",
                        dataType: "json",
                        headers: {
                            'Authorization': 'Bearer ' + '{{ $token }}',
